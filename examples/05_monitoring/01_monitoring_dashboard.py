...
from hother.cancelable import Cancelable, OperationRegistry, OperationStatus, cancelable
...
class OperationMonitor:
    ...
    def __init__(self, history_size: int = 1000):
        ...
        self.cancelation_reasons: Dict[str, int] = defaultdict(int)
    ...
    def record_operation(self, operation):
        ...
        # Track cancelation reasons
        if operation.cancel_reason:
            self.cancelation_reasons[operation.cancel_reason.value] += 1
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cancelation_reasons": dict(self.cancelation_reasons),
        }
...
class DashboardServer:
    ...
    async def update_loop(self, cancelable: Cancelable):
        ...
        while self.running:
            try:
                ...
                if stats["cancelation_reasons"]:
                    print("\n🛑 Cancelation reasons:")
                    for reason, count in sorted(stats["cancelation_reasons"].items(), key=lambda x: x[1], reverse=True):
    ...
    async def run(self):
        """Run the dashboard."""
        async with Cancelable.with_signal(signal.SIGINT, name="dashboard") as cancel:
    ...
    async def run_with_cancelable(self, cancelable: Cancelable):
        """Run the dashboard with an existing cancelable."""
        await self.update_loop(cancelable)
...
async def simulated_api_call(
    ...
    *,
    ...
    if cancelable:
        await cancelable.report_progress(f"Calling {endpoint}")
    ...
    if cancelable:
        await cancelable.report_progress(f"Completed {endpoint}")
...
@cancelable(register_globally=True, timeout=5.0)
...
async def workload_generator(monitor: OperationMonitor, cancelable: Cancelable):
    ...
    async def run_workload_with_error_handling(workload_func):
        ...
        try:
            ...
            # Don't crash - this is expected for simulated failures
            pass
...
async def example_dashboard():
    ...
    # Create shared cancelable for both dashboard and workload generator
    async with Cancelable.with_signal(signal.SIGINT, name="dashboard_and_workloads") as cancel:
        ...
        async with anyio.create_task_group() as tg:
            ...
            # Run dashboard
            await dashboard.run_with_cancelable(cancel)
...
async def example_api_monitoring():
    ...
    async with Cancelable.with_timeout(20.0, name="api_monitoring") as cancel:
        request_id = 0
        ...
        async def display_metrics():
            ...
            while True:
                ...
                # Check cancelation
                await cancel.token.check_async()
...
async def example_resource_monitoring():
    ...
    @cancelable(register_globally=True)
    async def cpu_intensive_task(task_id: int, cancelable: Optional[Cancelable] = None):
        ...
        for i in range(iterations):
            if i % 10000 == 0:
                if cancelable:
                    await cancelable.report_progress(f"Task {task_id}: {i}/{iterations} iterations")
                # Allow cancelation
                await anyio.sleep(0)
    ...
    async with Cancelable.with_timeout(30.0, name="resource_monitoring"):
...
async def main():
    ...
    print("Async Cancelation - Monitoring Dashboard Examples")

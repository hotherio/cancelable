#!/usr/bin/env python3
"""
Simplified database operation examples with cancellation support.
"""
...
async def create_test_data(engine, count: int = 500):
    ...
    async with AsyncSession(engine) as session:
        records = [DataPoint(name=f"sensor_{i % 10}", value=random.uniform(0, 100), timestamp=datetime.utcnow() - timedelta(minutes=i)) for i in range(count)]
...
async def example_basic_queries():
    """Example: Basic database queries with cancellation."""
    print("\n=== Basic Queries Example ===")

    engine = await setup_simple_database()
    await create_test_data(engine, 100)

    async with Cancellable.with_timeout(5.0, name="basic_queries") as cancel:
...
async def example_batch_processing():
    """Example: Process data in batches with cancellation."""
    print("\n=== Batch Processing Example ===")

    engine = await setup_simple_database()
    await create_test_data(engine, 200)

    @cancellable(timeout=10.0, name="batch_processor")
    async def process_batch(batch_size: int = 50, cancellable: Cancellable = None):
        async with cancellable_session(engine, cancellable) as session:
            ...
            while True:
                # Get next batch
                result = await session.execute(select(DataPoint).where(not DataPoint.processed).limit(batch_size))
                ...
                await cancellable.report_progress(f"Processed batch of {len(batch)} records", {"total_processed": processed_total})

                # Small delay to show cancellation works
...
async def example_with_progress():
    """Example: Long operation with progress reporting."""
    ...
    async with Cancellable().on_progress(capture_progress) as cancel:
        async with cancellable_session(engine, cancel) as session:
            ...
            for i, sensor in enumerate(sensors):
                ...
                if records:
                    ...
                    await cancel.report_progress(f"Sensor {sensor}: {len(records)} records, avg={avg_value:.2f}", {"sensor": sensor, "count": len(records), "average": avg_value})
...
async def example_cancellation():
    """Example: Operation that gets cancelled."""
    ...
    try:
        async with Cancellable.with_timeout(0.2, name="quick_timeout") as cancel:
            async with cancellable_session(engine, cancel) as session:
                # This should timeout
                for i in range(100):
...
async def main():
    """Run all examples."""
    print("Simplified Database Operations with Cancellation")
    print("==============================================")

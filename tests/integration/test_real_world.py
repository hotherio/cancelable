...
class TestIntegration:
    ...
    async def test_http_download_simulation(self):
        ...
        async def simulate_download(url: str, cancellable: Cancellable):
            ...
            for i in range(total_chunks):
                ...
                if (i + 1) % 5 == 0:
                    await cancellable.report_progress(f"Downloaded {i + 1}/{total_chunks} chunks", {"progress": (i + 1) / total_chunks * 100})
        ...
        async with Cancellable(name="download_test") as cancel:
        ...
        with pytest.raises(anyio.get_cancelled_exc_class()):
            async with Cancellable.with_timeout(0.3, name="timeout_download") as cancel:
    ...
    async def test_database_transaction_simulation(self):
        ...
        @cancellable(name="db_operation")
        async def insert_records(count: int, db: MockDatabase, cancellable: Cancellable = None):
            ...
            try:
                for i in range(count):
                    ...
                    if i % 10 == 0:
                        await cancellable.report_progress(f"Inserted {i}/{count} records")
    ...
    async def test_multi_stage_pipeline(self):
        ...
        async def stage1_fetch(cancellable: Cancellable) -> list:
            """Fetch data from source."""
            await cancellable.report_progress("Fetching data")
            data = []
            ...
            await cancellable.report_progress(f"Fetched {len(data)} items")
            return data

        async def stage2_process(data: list, cancellable: Cancellable) -> list:
            """Process the data."""
            await cancellable.report_progress("Processing data")
            processed = []
            ...
            await cancellable.report_progress(f"Processed {len(processed)} items")
            return processed

        async def stage3_save(data: list, cancellable: Cancellable) -> int:
            """Save the processed data."""
            await cancellable.report_progress("Saving data")
            saved_count = 0
            ...
            await cancellable.report_progress(f"Saved {saved_count} items")
            return saved_count

        # Test complete pipeline
        async with Cancellable(name="pipeline") as cancel:
            # Stage 1
        ...
        for cancel_after in [0.3, 0.8, 1.2]:
            ...
            async with anyio.create_task_group() as tg:
                ...
                try:
                    async with Cancellable.with_token(token, name="pipeline_cancel") as cancel:
                        raw_data = await stage1_fetch(cancel)
                        stage_reached = 1
    ...
    async def test_recursive_cancellation(self):
        ...
        async def recursive_operation(depth: int, cancellable: Cancellable):
            """Recursive operation that creates child operations."""
            execution_order.append(f"start_{depth}")

            if depth > 0:
                # Create child operation
                async with Cancellable(name=f"level_{depth}", parent=cancellable) as child:
                    await anyio.sleep(0.05)
        ...
        async with Cancellable(name="root") as root:
        ...
        execution_order.clear()
        root_cancel = Cancellable(name="root")
    ...
    async def test_real_world_scenario(self):
        ...
        class WebScraper:
            ...
            async def scrape_site(self, base_url: str, page_count: int, cancellable: Cancellable = None):
                ...
                for page_num in range(1, page_count + 1):
                    ...
                    try:
                        ...
                        async with cancellable.shield() as shielded:
                        ...
                        await cancellable.report_progress(f"Error scraping {page_url}: {e}")
            ...
            async def _scrape_page(self, url: str, cancellable: Cancellable) -> list:
        ...
        async def monitor_and_cancel():
            ...
            for op in ops:
                if "scrape_site" in op.name and op.duration_seconds > 0.25:
                    await token.cancel(CancellationReason.TIMEOUT, "Scraping taking too long")

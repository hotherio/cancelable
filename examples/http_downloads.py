#!/usr/bin/env python3
"""
HTTP download examples with cancellation support.
"""
...
async def example_simple_download():
    ...
    async with Cancellable.with_timeout(30.0, name="simple_download") as cancel:
...
async def example_multi_file_download():
    ...
    async def download_with_id(url: str, path: str, size: str, file_id: int):
        ...
        cancellable = Cancellable.with_token(token, name=f"download_{file_id}").combine(Cancellable.with_timeout(60.0))

        cancellable.on_progress(lambda op_id, msg, meta: print(f"  File {file_id} ({size}): {msg}"))

        try:
            async with cancellable:
                bytes_down = await download_file(url, path, cancellable)
                return file_id, bytes_down, Path(path)
        except Exception as e:
            logger.error(f"Download {file_id} failed: {e}")
...
async def example_resumable_download():
    ...
    async with anyio.create_task_group() as tg:
        ...
        try:
            async with Cancellable.with_token(token1, name="download_attempt_1") as cancel:
    ...
    if output_path.exists():
        ...
        async with Cancellable.with_timeout(30.0, name="download_resume") as cancel:
            cancel.on_progress(lambda op_id, msg, meta: print(f"  {msg}"))
...
async def example_parallel_chunks():
    ...
    async def download_chunk(
        ...
        async with CancellableHTTPClient(cancellable) as client:
            ...
            await cancellable.report_progress(f"Chunk {chunk_id} downloaded", {"size": len(data), "range": f"{start}-{end}"})
    ...
    async with Cancellable.with_timeout(30.0, name="parallel_download") as cancel:
        cancel.on_progress(lambda op_id, msg, meta: print(f"  {msg}"))
...
async def example_download_with_retry():
    ...
    class FlakyDownloader:
        ...
        async def download(
            ...
            # Actual download
            return await download_file(url, output_path, cancellable)
    ...
    for attempt in range(max_retries):
        try:
            ...
            async with Cancellable.with_timeout(30.0, name=f"download_retry_{attempt}") as cancel:
                cancel.on_progress(lambda op_id, msg, meta: print(f"    {msg}"))
...
async def example_download_queue():
    """Example: Download queue with cancellation."""
    print("\n=== Download Queue ===")
    ...
    # Process queue with global cancellation
    ...
    async def process_download(item: dict):
        ...
        try:
            async with cancellable:
                await cancellable.report_progress(f"Starting download {item['id']}")
                ...
                await cancellable.report_progress(f"Completed download {item['id']}", {"bytes": bytes_down})
    ...
    try:
        async with Cancellable.with_signal(signal.SIGINT, name="queue_manager"):
...
async def main():
    ...
    print("Async Cancellation - HTTP Download Examples")
    print("==========================================")

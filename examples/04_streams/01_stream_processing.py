#!/usr/bin/env python3
"""
Stream processing examples with async cancelation.
"""
...
async def example_basic_stream_processing():
    """Example: Basic stream processing with cancelation."""

    # Process sensor stream with timeout
    async with Cancelable.with_timeout(5.0, name="sensor_processing") as cancel:
...
async def example_multi_stream_merge():
    """Example: Merge multiple streams with cancelation."""
    ...
    async with Cancelable.with_timeout(5.0, name="multi_sensor") as cancel:
        reading_counts = {}
...
async def example_stream_transformation():
    ...
    async def validate_reading(data: dict) -> dict | None:
        ...
        if not (0 <= data["temperature"] <= 50):
            print(f"  ⚠️  Temperature out of range: {data['temperature']}")
            return None

        if not (0 <= data["humidity"] <= 100):
            print(f"  ⚠️  Humidity out of range: {data['humidity']}")
            return None
    ...
    async with cancelable:
        ...
        async for reading in cancelable.stream(pipeline, report_interval=20):
            comfort_stats[reading["comfort"]] += 1

            # Print sample readings
            if sum(comfort_stats.values()) % 10 == 0:
                print(
                    f"  🌡️  T: {reading['temperature']}°C, H: {reading['humidity']}%, DP: {reading['dew_point']}°C, Comfort: {reading['comfort']}"
                )
...
async def example_chunked_processing():
    ...
    # Process in chunks
    async with Cancelable.with_timeout(10.0, name="chunked_processing") as cancel:
        stream = sensor_data_stream("SENSOR-001", interval=0.05)

        batch_count = 0
        async for chunk in chunked_cancellable_stream(stream, chunk_size=20, cancelable=cancel):
            batch_count += 1
...
async def example_backpressure_handling():
            ...
            async with send_stream:
                async for item in stream:
                    try:
                        ...
                        print("  ⚠️  Buffer full, dropping reading")
    ...
    async with anyio.create_task_group() as tg:
        ...
        try:
            async with Cancelable.with_token(token, name="backpressure_demo") as cancel:
                stream = sensor_data_stream("SENSOR-001", interval=0.05)
                processed_count = 0
                ...
                async for reading in cancel.stream(buffered_processing(stream)):
                    ...
                    expected_readings = (
                        datetime.fromisoformat(reading["processed_at"]) - datetime.fromisoformat(reading["timestamp"])
                    ).total_seconds() / 0.05

                    if expected_readings > 1.5:
                        print("  ⚠️  Possible dropped readings due to slow processing")
...
async def example_stateful_stream_processing():
    """Example: Stateful stream processing with cancelation."""
    ...
    class TemperatureMonitor:
        ...
        async def process(self, reading: dict) -> dict | None:
    ...
    async with Cancelable.with_timeout(10.0, name="anomaly_detection") as cancel:
        ...
        async def anomaly_stream():
            ...
            async for reading in sensor_data_stream("SENSOR-001", interval=0.1, error_rate=0):
                ...
                if normal_count % 30 == 0:
                    ...
                    print(f"  Injected anomaly: {reading['temperature']}°C")
